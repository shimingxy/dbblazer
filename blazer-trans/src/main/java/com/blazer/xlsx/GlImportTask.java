 ...
 *
 */
public class GlImportTask  implements PipeLineTask{
	private static final Logger _logger = LoggerFactory.getLogger(GlImportTask.class);
	...
	public int execute() throws Exception {
		...
		for(String xlsxFileName :filePathList) {
			System.out.println("===========filename "+xlsxFileName);
	...
	public void readFile(String xlsxName) throws Exception {
		...
		_logger.info("==========="+entName);
		int i=0;
		Statement stmt=sourcConn.createStatement();
		//String gx="�ɶ�";
		for (Row rows : sheet) {
			if(i==0) {i++;continue;}//������һ�м�¼
			
			if("������".equals(rows.getCell(0).toString())) {//���������¼
				i++;continue;
			}

			stmt.execute("INSERT INTO GS_TEST_SZGL (TABLE_ENTNAME,ENTNAME,RELATION,SJJD,JDTYPE,TABLE_REGNO,REGCAP,HYML,CLDATE) VALUES("
					+"'"+entName+"',"
					+"'"+rows.getCell(0)+"',"
					+"'"+rows.getCell(1)+"',"
					+"'"+rows.getCell(2)+"',"

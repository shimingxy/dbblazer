 ...
 * 
 */
public class TransBase implements Runnable{
	private static final Logger _logger = LoggerFactory.getLogger(TransBase.class);
	protected DataSource sourceDataSource;
	protected DataSource targetDataSource;
	protected int commitNum = 2000;
	protected long recordsCount = 0;
	protected long insertRecordsCount = 0;
	protected long updateRecordsCount = 0;
	protected long deleteRecordsCount = 0;
	protected String insertSql=null;
	protected String updateLobSql=null;
	protected boolean hasLob=false;
	protected String transType;
	protected String owner;
	protected TableDescribe table;
	
	protected int threadNum=0;
	protected ArrayList<TableColumns> listTableColumns ;
	protected ArrayList<TableDescribe> listTables ;
	protected ArrayList<String> listTablePKs ;
	protected ArrayList<TableColumns> listTableLobColumns ;
	...
	public void batchInsert(ResultSet rs) throws Exception{
		...
		while(rs.next()){
			...
			for(TableColumns tc : listTableColumns){
				...
				if(tc.getDataType().equalsIgnoreCase("character varying")||
						tc.getDataType().equalsIgnoreCase("character")||
						tc.getDataType().equalsIgnoreCase("text")||
				...
				}else if(tc.getDataType().equalsIgnoreCase("numeric")||
						tc.getDataType().equalsIgnoreCase("integer")
						){
	...
	public void buildInsertSql(){
		...
		updateLobSql=updateLobSql.substring(0, updateLobSql.length()-1)+" FROM "+table.getTableName()+" WHERE "+where_sql.substring(0, where_sql.length()-4)+" FOR UPDATE";

 ...
 *
 */
public class DDLOracle2Oracle extends DDLExportBase{
	private static final Logger _logger = LoggerFactory.getLogger(DDLOracle2Oracle.class);
	
	/**
	 * 
	 */
	public DDLOracle2Oracle() {
	...
	public void  readTable(String tableName) throws Exception{
		...
		if(TABLECOMMENTS!=null&&!TABLECOMMENTS.equals("")){
			out.write(("comment on table  "+String.format("%-40s", tableName.toUpperCase())+" is '"+TABLECOMMENTS+"';\r\n\r\n").getBytes());
		}
		for (int i=0;i<tcArray.size();i++){
			TableColumns tc=tcArray.get(i);
			if(tc.getColumnComments()!=null&&!tc.getColumnComments().equals("")){
				out.write(("comment on column "+String.format("%-40s", tableName.toUpperCase()+"."+tc.getColumnName())+" is '"+tc.getColumnComments()+"';\r\n").getBytes());
			}
		}
		out.write("\r\n".getBytes());
		rs.close();
